SELECT
    ...
    ISNULL(
        ...
        FROM
                ...
                FROM
                    (
                        ...
                        WHERE
                            ...
                            AND (
                                A.INDICATION_FOR_USE__C = 'Heart Failure - Reduced Ejection Fraction'
                            )
            ...
            LEFT OUTER JOIN dbo.sfdcContact AS Q ON M.PHYSICIAN_ID = Q.ID
            LEFT OUTER JOIN dbo.sfdcAccount AS W ON Q.ACCOUNTID = W.ID
            LEFT OUTER JOIN dbo.qryRoster AS R ON R.REP_ID = W.OWNERID
            AND R.[isLATEST?] = 1
            LEFT OUTER JOIN dbo.qryRoster_RM AS RR ON RR.USER_ID = W.OWNERID
            LEFT OUTER JOIN (
                SELECT
                    PHYSICIAN_ID,
                    IMPLANTED_YYYYMM,
                ...
                WHERE
                    ...
                    AND (
                        STAGENAME IN ('Implant Completed', 'Revenue Recognized')
                    )
                    AND (ISIMPL = 1)
                GROUP BY
                    PHYSICIAN_ID,
