...
,
    ...
    FROM
        ...
        LEFT JOIN tblRates_TM Z ON A.TERRITORY_ID = Z.TERRITORY_ID
        AND A.ROLE = 'REP'
        LEFT JOIN tblFCE_COMP F ON A.EMP_EMAIL = f.FCE_EMAIL
    WHERE
        (
            A.ROLE IN ('FCE', 'REP', 'MDR', 'ATM', 'MDM', 'CEA')
        )
)
SELECT
    R.*,
    CASE
        WHEN R.TENNURE_MONTHS BETWEEN 0
        AND 6 THEN '0 - 6 mos'
        WHEN R.TENNURE_MONTHS BETWEEN 7
        AND 12 THEN '07 - 12 mos'
        WHEN R.TENNURE_MONTHS BETWEEN 13
        AND 18 THEN '13 - 18 mos'
        WHEN R.TENNURE_MONTHS BETWEEN 19
        AND 24 THEN '19 - 24 mos'
        WHEN R.TENNURE_MONTHS > 24 THEN '25+ mos'
        ELSE '????'
    END AS [TENNURE_BIN]
FROM
    R